...
VocabularyResultsDialog::VocabularyResultsDialog(
    ...
    summaryLabel->setStyleSheet("color: #34495e; padding: 10px 0;");
    resultsLayout->addWidget(summaryLabel);
    ...
    if (expectingRomaji) {
        ...
        QLabel *romajiLabel = new QLabel(
            QString("📝 Romaji: %1 correct, %2 incorrect (%3%)")
            .arg(correctRomajiCount)
            .arg(incorrectRomajiCount)
            .arg(QString::number(romajiPercent, 'f', 1)),
            this
        );
        romajiLabel->setStyleSheet("font-size: 14px; padding: 5px; background-color: #ecf0f1; border-radius: 4px; margin: 2px;");
        resultsLayout->addWidget(romajiLabel);
    }
    ...
    if (expectingEnglish) {
        ...
        QLabel *englishLabel = new QLabel(
            QString("🇺🇸 English: %1 correct, %2 incorrect (%3%)")
            .arg(correctEnglishCount)
            .arg(incorrectEnglishCount)
            .arg(QString::number(englishPercent, 'f', 1)),
            this
        );
        englishLabel->setStyleSheet("font-size: 14px; padding: 5px; background-color: #ecf0f1; border-radius: 4px; margin: 2px;");
        resultsLayout->addWidget(englishLabel);
    }
    ...
    if (totalAnswers > 0) {
        ...
        if (overallPercent < 70) color = "#e74c3c"; // Red for poor
        else if (overallPercent < 85) color = "#f39c12"; // Orange for okay
        
        accuracyLabel->setStyleSheet(QString("color: %1; padding: 10px; background-color: #f8f9fa; border-radius: 6px; margin: 5px;").arg(color));
        resultsLayout->addWidget(accuracyLabel);
    }
    
    // Words to practice more
    if (!incorrectWords.empty()) {
        resultsLayout->addSpacing(15);
        ...
        practiceLabel->setStyleSheet("color: #e74c3c; padding: 10px 0;");
        resultsLayout->addWidget(practiceLabel);
        ...
        for (const auto &pair : sortedIncorrect) {
            ...
            resultsLayout->addWidget(wordLabel);
        }
    } else {
        resultsLayout->addSpacing(15);
        ...
        resultsLayout->addWidget(perfectLabel);
    }
